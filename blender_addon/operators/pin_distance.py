...
class PC_OT_SnapPinToDistance(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context) -> set:
        ...
        geometry_matrix = np.array(tracker.geometry.matrix_world)
        pin_pos_world = (geometry_matrix @ np.append(pin_pos_local, homogeneous_w))[:3]
        ...
        geometry_matrix_inv = np.linalg.inv(geometry_matrix)
        new_pin_pos_local = (geometry_matrix_inv @ np.append(new_pin_pos_world, homogeneous_w))[:3]
